...
class 游客信息(Base):
    ...
    游客ID = Column(Integer, primary_key=True, comment="游客唯一标识")
    姓名 = Column(String(50), nullable=False, comment="游客姓名")
    身份证号码 = Column(String(18), nullable=False, unique=True, comment="18位身份证号")
    联系方式 = Column(String(11), nullable=False, comment="11位手机号")
    入园时间 = Column(DateTime, comment="实际入园时间")
    离园时间 = Column(DateTime, comment="实际离园时间")
    入园方式 = Column(String(10), nullable=False, comment="线上预约/现场购票")

class 预约记录(Base):
    __tablename__ = "预约记录"

    预约编号 = Column(Integer, primary_key=True, comment="预约唯一标识")
    游客ID = Column(Integer, ForeignKey("游客信息.游客ID"), nullable=False, comment="关联游客信息")
    预约时间 = Column(DateTime, nullable=False, comment="预约提交时间")
    入园时段 = Column(String(20), nullable=False, comment="如8:00-12:00")
    同行人数 = Column(Integer, nullable=False, comment="同行人数（含本人）")
    预约状态 = Column(String(10), nullable=False, comment="已确认/已取消/已完成")
    购票金额 = Column(Numeric(10, 2), nullable=False, comment="购票总金额")
    支付状态 = Column(String(10), nullable=False, comment="已支付/未支付")

class 流量控制信息(Base):
    __tablename__ = "流量控制信息"

    信息ID = Column(Integer, primary_key=True, comment="流量记录唯一标识")
    当前状态 = Column(String(10), nullable=False, comment="正常/预警/限流")
    实时在园人数 = Column(Integer, nullable=False, comment="当前区域在园人数")
    区域编号 = Column(Integer, nullable=False, comment="园区区域编号")
    日最大承载量 = Column(Integer, nullable=False, comment="区域日最大承载人数")
    预警阈值 = Column(Integer, nullable=False, comment="触发预警的人数阈值")

class 游客轨迹数据(Base):
    __tablename__ = "游客轨迹数据"

    轨迹编号 = Column(Integer, primary_key=True, comment="轨迹唯一标识")
    游客ID = Column(Integer, ForeignKey("游客信息.游客ID"), nullable=False, comment="关联游客信息")
    信息ID = Column(Integer, ForeignKey("流量控制信息.信息ID"), nullable=False, comment="关联流量控制信息")
    定位时间 = Column(DateTime, nullable=False, comment="轨迹定位时间")
    经度 = Column(Numeric(9, 6), nullable=False, comment="定位经度")
    纬度 = Column(Numeric(9, 6), nullable=False, comment="定位纬度")
    是否超出规定路线 = Column(String(2), nullable=False, comment="是/否")
...
# 游客智能管理只读视图（按需配置，示例为流量统计视图）
READ_ONLY_VIEWS.update({
    "区域流量统计视图": 流量控制信息,  # 示例：流量控制信息表作为只读视图
    "游客违规轨迹视图": 游客轨迹数据   # 示例：违规轨迹仅可查看不可修改
})
...
def query_tourist_flow_by_area(session, 区域编号: int):
    """
    按区域编号查询实时流量（管理人员核心操作）
    :param session: 数据库会话
    :param 区域编号: 园区区域编号
    :return: 该区域流量信息
    """
    try:
        flow_info = session.query(流量控制信息).filter(
            流量控制信息.区域编号 == 区域编号
        ).order_by(流量控制信息.信息ID.desc()).first()

        if not flow_info:
            print(f"区域编号 {区域编号} 无流量数据")
            return None

        # 封装返回结构化数据
        return {
            "区域编号": flow_info.区域编号,
            "实时在园人数": flow_info.实时在园人数,
            "当前状态": flow_info.当前状态,
            "日最大承载量": flow_info.日最大承载量,
            "预警阈值": flow_info.预警阈值,
            "超载率": f"{(flow_info.实时在园人数 / flow_info.日最大承载量 * 100):.2f}%"
        }
    except Exception as e:
        print(f"查询区域流量失败：{e}")
        return None
...
def add_tourist_reservation(session, reservation_data: dict):
    """
    新增游客预约记录（游客核心操作）
    :param session: 数据库会话
    :param reservation_data: 预约数据字典
    :return: 成功/失败
    """
    try:
        # 校验游客是否存在
        tourist = session.query(游客信息).filter(
            游客信息.游客ID == reservation_data["游客ID"]
        ).first()
        if not tourist:
            print("游客ID不存在，无法创建预约")
            return "error"

        # 创建预约记录
        new_reservation = 预约记录(**reservation_data)
        session.add(new_reservation)
        session.commit()

        # 联动更新流量（示例：预约成功后预占流量）
        flow_info = session.query(流量控制信息).filter(
            流量控制信息.区域编号 == 1  # 示例默认区域1，可按需调整
        ).first()
        if flow_info:
            flow_info.实时在园人数 += reservation_data["同行人数"]
            # 自动更新流量状态
            if flow_info.实时在园人数 >= flow_info.预警阈值:
                flow_info.当前状态 = "预警"
            if flow_info.实时在园人数 >= flow_info.日最大承载量:
                flow_info.当前状态 = "限流"
            session.commit()

        print(f"预约编号 {new_reservation.预约编号} 创建成功")
        return "success"
    except Exception as e:
        session.rollback()
        print(f"创建预约失败：{e}")
...
def query_tourist_violation(session, 游客ID: int):
    """
    查询游客违规轨迹（管理人员核心操作）
    :param session: 数据库会话
    :param 游客ID: 游客唯一标识
    ...
    """
    try:
        violation_tracks = session.query(游客轨迹数据).filter(
            游客轨迹数据.游客ID == 游客ID,
            游客轨迹数据.是否超出规定路线 == "是"
        ).order_by(游客轨迹数据.定位时间.desc()).all()

        if not violation_tracks:
            print(f"游客ID {游客ID} 无违规轨迹")
            return []
        ...
        for track in violation_tracks:
            result.append({
                "轨迹编号": track.轨迹编号,
                "定位时间": track.定位时间.strftime("%Y-%m-%d %H:%M:%S"),
                "经度": float(track.经度),
                "纬度": float(track.纬度),
                "所属区域": session.query(流量控制信息).filter(
                    流量控制信息.信息ID == track.信息ID
                ).first().区域编号
            })
        return result
    except Exception as e:
    ...
    while True:
        ...
        with get_db_session() as session:
            ...
            # 处理游客管理专属操作
                ...
                area_id = input("请输入区域编号：")
                try:
                    area_id = int(area_id)
                    flow_data = query_tourist_flow_by_area(session, area_id)
                    if flow_data:
                        print("\n===== 区域流量信息 =====")
                ...
                reservation_data = {
                    "游客ID": int(input("游客ID：")),
                    "预约时间": datetime.strptime(input("预约时间（格式：2025-01-01 10:00:00）："), "%Y-%m-%d %H:%M:%S"),
                    "入园时段": input("入园时段（如8:00-12:00）："),
                    "同行人数": int(input("同行人数：")),
                    "预约状态": "已确认",
                    "购票金额": float(input("购票金额：")),
                    "支付状态": input("支付状态（已支付/未支付）：")
                }
                add_tourist_reservation(session, reservation_data)
            elif choice == "8":
                tourist_id = input("请输入游客ID：")
                try:
                    tourist_id = int(tourist_id)
                    violation_data = query_tourist_violation(session, tourist_id)
                    if violation_data:
                        print("\n===== 游客违规轨迹 =====")
                        for track in violation_data:
                            print(track)
                except ValueError:
                    print("游客ID必须为数字")
            else:
...
# ========== 流量控制触发器（事件监听） ==========
@event.listens_for(流量控制信息.实时在园人数, 'set')
def check_flow_limit(target, value, oldvalue, initiator):
    """
    监听实时在园人数变更，自动更新流量状态
    """
    if value >= target.日最大承载量:
        target.当前状态 = "限流"
    elif value >= target.预警阈值:
        target.当前状态 = "预警"
    else:
        target.当前状态 = "正常"
    return value
